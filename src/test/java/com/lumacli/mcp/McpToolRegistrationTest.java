...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("lumacli.audit.dir");
        System.setProperty("lumacli.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("lumacli.audit.dir");
            } else {
                System.setProperty("lumacli.audit.dir", previous);
            }
        }
    }
