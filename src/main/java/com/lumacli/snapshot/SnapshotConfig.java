...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("lumacli.snapshot.enabled", "LUMACLI_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("lumacli.snapshot.dir", "LUMACLI_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".lumacli", "snapshots").toString()));
        int max = readInt("lumacli.snapshot.max", "LUMACLI_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("lumacli.snapshot.excludes", "LUMACLI_SNAPSHOT_EXCLUDES", ""));
