 ...
 * 1. 系统属性：{@code lumacli.react.token.budget} / {@code lumacli.react.stagnation.window} /
 *    {@code lumacli.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dlumacli.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dlumacli.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("lumacli.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("lumacli.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("lumacli.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
