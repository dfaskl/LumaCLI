...
import com.lumacli.llm.LlmTraceLogger;
import com.lumacli.context.ContextProfile;
import com.lumacli.context.TokenUsageFormatter;
import com.lumacli.lsp.LspDiagnosticReport;
import com.lumacli.memory.ConversationHistoryCompactor;
import com.lumacli.memory.ExplicitMemoryHints;
import com.lumacli.memory.MemoryManager;
import com.lumacli.prompt.PromptAssembler;
import com.lumacli.prompt.PromptContext;
import com.lumacli.prompt.PromptMode;
import com.lumacli.prompt.ProjectMemoryLoader;
...
import com.lumacli.skill.SkillIndexFormatter;
import com.lumacli.skill.SkillRegistry;
import com.lumacli.util.AnsiStyle;
import com.lumacli.tool.ToolRegistry;
import com.lumacli.tool.ToolRegistry.ToolExecutionResult;
import com.lumacli.tool.ToolRegistry.ToolInvocation;
import com.lumacli.util.TerminalMarkdownRenderer;
...
public class Agent {
    ...
    public String getContextStatus() {
        com.lumacli.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.lumacli.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.lumacli.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.lumacli.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.lumacli.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.lumacli.memory.MemoryEntry.estimateTokens(
